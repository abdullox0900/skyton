...
const Header = () => {
    ...
    return (
        <header className="text-white w-full min-[1900px]:bg-nightshade max-[1024px]:bg-nightshade rounded-bl-[25px] rounded-br-[25px]">
            ...
            <div className="hidden lg:flex flex-col gap-4">
                <div className="container mx-auto py-[37px] xl:py-[15px] flex items-center justify-between relative z-10">
                    <Link href="/" className="hover:opacity-80 transition-opacity">
                        <Image src={'/assets/svg/logo.svg'} alt="header-logo" width={176} height={46} />
                    </Link>

                    <div className="text-[32px] font-bold ml-[170px] xl:text-[20px]">Telegram NFT продукты</div>
                    ...
                    <div className='flex items-center gap-[20px]'>
                        ...
                        <button className='flex items-center gap-[20px] py-[13px] px-[37px] text-[20px] text-[#e0e5ec] font-medium bg-gradient-to-r from-[#335d96] to-[#4881cf] rounded-full hover:opacity-80 transition-opacity active:scale-95'>
                            <Image src={'/assets/svg/telegram-icon.svg'} alt="header-button" width={30} height={26} />
                            Авторизация
                        </button>
                    </div>
                </div>
                <ul className='flex items-center gap-[36px] justify-center relative z-10 mt-[10px]'>
                    {navItems.map((item) => {
                        const isActive = pathname === item.path || (item.path === '/nicknames' && pathname === '/')
                        return (
                            <li key={item.path} className="relative">
                                <Link
                                    href={item.path}
                                    className={`text-[20px] font-light transition-colors ${isActive ? 'text-white' : 'text-[#A7AFBA] hover:text-white/80'}`}
                                >
                                    {item.name}
                                </Link>
                                {isActive && (
            ...
            <div className="lg:hidden">
                <div className="container mx-auto px-4 py-6 relative z-10">
                    ...
                    <div className="flex items-center justify-between mb-4 max-[550px]:mb-0 ">
                        <Link href="/" className="hover:opacity-80 transition-opacity">
                            <Image src={'/assets/svg/logo.svg'} alt="header-logo" width={120} height={32} />
                        </Link>
                        ...
                        >
                            <div className="absolute inset-0 flex items-center justify-center">
                                <div className="w-6 h-6 relative">
                                    <span className={`absolute top-0 left-0 w-full h-0.5 bg-white transition-all duration-300 ${isMobileMenuOpen ? 'top-[11px] rotate-45' : ''}`}></span>
                                    <span className={`absolute top-[11px] left-0 w-full h-0.5 bg-white transition-all duration-300 ${isMobileMenuOpen ? 'opacity-0' : ''}`}></span>
                                    <span className={`absolute bottom-0 left-0 w-full h-0.5 bg-white transition-all duration-300 ${isMobileMenuOpen ? 'bottom-[11px] -rotate-45' : ''}`}></span>
                                </div>
                            </div>
                        </button>
                    </div>
                    ...
                    <div className="text-center max-[550px]:hidden">
                        <div className="text-xl md:text-2xl font-bold">Telegram NFT продукты</div>
                    </div>

                    {/* Navigation items */}
                    <div className={`transition-all duration-300 ${isMobileMenuOpen ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
                        <div className='flex flex-col items-center gap-4 py-4'>
                            ...
                            <button className='flex items-center gap-3 py-3 px-6 text-lg text-[#e0e5ec] font-medium bg-gradient-to-r from-[#335d96] to-[#4881cf] rounded-full hover:opacity-80 transition-opacity active:scale-95 w-full max-w-xs justify-center'>
                                <Image src={'/assets/svg/telegram-icon.svg'} alt="header-button" width={24} height={20} />
                                Авторизация
                            </button>
                                ...
                                {navItems.map((item) => {
                                    ...
                                    return (
                                        <li key={item.path} className="w-full text-center relative">
                                            <Link
                                                href={item.path}
                                                className={`text-[18px] font-light block py-2 transition-colors ${isActive
                                                    ? 'text-white'
